  ...
  suite('Integration tests with chai-http', function() {
    ...
    suite('GET /hello?name=[name] => "hello [name]"', function(){
      ...
      test('Test GET /hello with your name',  function(done){ // Don't forget the callback...
         chai.request(server)             // 'server' is the Express App
          .get('/hello?name=SHS') /** <=== Put your name in the query **/
          .end(function(err, res){        // res is the response object
            ...
            assert.equal(res.status, 200);
            assert.equal(res.text, 'hello SHS');
            done();   // Always call the 'done()' callback when finished.
          });
      });
      ...
      test('send {surname: "Colombo"}',  function(done){
       ...
       chai.request(server)
        ...
        .end(function(err, res){
          ...
          assert.equal(res.body.name, 'Cristoforo', 'This is the name');
          assert.equal(res.body.surname, 'Colombo', 'This is the surname');
      ...
      test('send {surname: "da Verrazzano"}', function(done) {
          chai.request(server)
              ...
              .end(function (err, res) {
                  ...
                  assert.equal(res.status, 200, 'this is teh expected response');
                  assert.equal(res.type, 'application/json', 'json type expected');

                  assert.equal(res.body.name, 'Giovanni', 'this is the expected name');
                  assert.equal(res.body.surname, 'da Verrazzano', 'this is the expected surname');
                  assert.equal(res.body.dates, '1485 - 1528', 'this is the expected date range');
